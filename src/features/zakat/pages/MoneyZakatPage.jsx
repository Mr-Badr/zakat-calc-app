...
const CTA = ({ cta, type }) => (
  <div className="my-8 text-center">
    <a
      href={`/?type=${type}`}
      className="inline-block px-6 py-3 bg-emerald-600 text-white rounded-lg text-lg font-bold shadow hover:bg-emerald-700 transition-all duration-200"
    >
      {cta || 'Start Your Calculation Now'}
    </a>
  </div>
);

const MoneyZakatPage = ({ content, type }) => {
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      {/* Hero/Intro */}
      <h1 className="text-3xl font-bold mb-4 flex items-center gap-2">💵 {content.name || 'Money Zakat'}</h1>
